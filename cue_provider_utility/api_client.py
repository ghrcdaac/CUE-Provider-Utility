...
# Logging functions for verbose debugging (-vv)
    ...
    async def _request(
        ...
        try:
            ...
            if response.status_code not in expected_status_codes:
                ...
                try:
                    # Try to get a clean error message from the backend's JSON response
                    api_error = APIErrorResponse.model_validate_json(response.content)
                    # This is the detailed message from the backend (e.g., "Not authenticated")
                    error_message = f"API Error (HTTP {response.status_code}): {api_error.error.message}"
                except Exception:
                    # If parsing fails, use the HTTP reason phrase (e.g., "Unauthorized", "Gateway Timeout")
                    error_message = f"API request failed with status {response.status_code} ({response.reason_phrase})"
                
                # We will now raise an exception with this detailed message.
    ...
    async def upload_part_to_s3_presigned_put(self, url: str, part_data: bytes, content_length: int) -> httpx.Response:
        ...
        try:
            ...
            if response.status_code != 200:
                raise APIRequestError(f"S3 presigned PUT for part upload failed with status {response.status_code}.",
                                      status_code=response.status_code, response_content=response.text)
            return response
        except httpx.HTTPError as e:
