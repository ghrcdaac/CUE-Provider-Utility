...
class ApiClient:
    ...
    async def _request(
        ...
        try:
            response = await self.http_client.request(
                method, full_url, json=json_payload,
            )
            ...
            if response.status_code not in expected_status_codes:
                ...
                raise APIRequestError(
                    message=error_message, status_code=response.status_code, response_content=error_content
                )

            if response_model:
                if response.status_code == 204: 
                    return None # For 204 No Content, no body to parse
    ...
    async def get_presigned_url_single(self, payload: InitiateUploadRequest) -> InitiateUploadResponse:
        ...
        return await self._request(
            "POST", endpoint, json_payload=payload.model_dump(exclude_none=True),
    ...
    async def confirm_single_upload(self, payload: ConfirmSingleUploadRequest) -> None: # Or a specific response model
        ...
        await self._request(
            ...
            expected_status_codes=[200, 201] # Or whatever backend returns on success
        )
    ...
    async def complete_multipart_upload(self, payload: MultipartCompleteRequest) -> MultipartCompleteResponse:
        ...
        # The payload (MultipartCompleteRequest) now includes final_file_size, collection_path, content_type
        return await self._request(
            "POST", endpoint, json_payload=payload.model_dump(exclude_none=True), 
    ...
    async def upload_to_s3_presigned_post(self, url: str, fields: Dict[str, str], file_path: Path, file_name: str, content_type: str) -> httpx.Response:
        # Ensure file is opened in binary mode for httpx files parameter
        with open(file_path, 'rb') as f:
            files_data = {'file': (file_name, f, content_type)}
            logger.debug(f"S3 POST: URL={url}, Fields={list(fields.keys())}, File={file_name}")
            try:
